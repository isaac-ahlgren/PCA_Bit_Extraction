...
struct eig_decomp_args* alloc_eig_args(uint32_t dim_size, uint32_t eig_vec_num, uint32_t execs, float err_tol)
{
    ...
    args->eig_vectors = malloc(sizeof(float)*dim_size*eig_vec_num);
...
void eig_decomp(float* matrix, int* convergence, uint32_t max_shift, struct eig_decomp_args* args) {
    ...
    for(k = 0; k < nvecs; k++) {
        ...
        //printf("S before deflation, round %d\n", k);
        //print_matrix(matrix, args->dim_size, args->dim_size);
        ...
        //printf("Deflated S, round %d\n", k);
        //print_matrix_sci(matrix, args->dim_size, args->dim_size);
